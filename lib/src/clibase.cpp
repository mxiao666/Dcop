...
int cliMgr::Report(RspMsg *outMessage, int cmd)
{
    ...
    {
        len = snprintf(buf, maxlen, "%-*s", ptbl->tblBody[i].fmrlen, ptbl->tblBody[i].item);
        _outinfo.Append((const BYTE *)buf, len);
        totallen += ptbl->tblBody[i].fmrlen;
    }
    len = snprintf(buf, maxlen, "\n%*s\n%s\n", totallen / 2, ptbl->tblName, _outinfo.GetBuff());
    ...
    len = snprintf(buf, maxlen, "Command response %d line record\n", outMessage->count);
    outinfo.Append((const BYTE *)buf, len);
...
int cliMgr::Process(int fd, char *cmdbuffer, int len, bool &isExit)
{
    ...
    if (OS::equal((char *)cmd.GetBuff(), "q", 1) ||
        OS::equal((char *)cmd.GetBuff(), "quit", 4) ||
        OS::equal((char *)cmd.GetBuff(), "e", 1) ||
        OS::equal((char *)cmd.GetBuff(), "exit", 4))
    {
        isExit = true;
        iRet = RET_OK;
        goto _end;
    ...
    if (OS::equal((char *)cmd.GetBuff(), "dump", 4))
    {
    ...
    if (OS::equal((char *)cmd.GetBuff(), "man", 3) ||
        OS::equal((char *)cmd.GetBuff(), "help", 4))
    {
...
public:
    ...
    virtual int Init()
    {
        ...
        m_cliMgr = reinterpret_cast<cliMgr *>(g_objKernel->InterFace(MODELU_CLI));
        g_objKernel->Init([]() -> void {
            telnet *obj = reinterpret_cast<telnet *>(g_objKernel->InterFace(MODELU_TELNET));
            if (obj != nullptr)
                obj->Process();
        });
        return RET_OK;
        ...
        while (!m_isExit)
        {
            ...
            for (int i = 0; i < nfds; ++i) // for循环中可以修改为线程池处理epoll事件
            {
                if (events[i].data.fd == m_listenFd)
                {
                    ...
                    int sockfd = accept(m_listenFd, (sockaddr *)&clientaddr, &clilen);
                    if (sockfd < 0)
                    {
                        continue;
                    }
                ...
                else if (events[i].events & EPOLLIN)
                {
                    ...
                    if (len < 0)
                    {
                        close(sockfd);
                        events[i].data.fd = -1;
                        LOG_ERROR("worker read data error = %s.", strerror(errno));
                    }
                    else
                    {
                        buf[len - 1] = '\0';
                        m_cliMgr->Process(events[i].data.fd, buf, strlen(buf), isExit);
                    }
                }
            }
        }
